...
def main():
    ...
    counter=0
    print("pad token is",tokenizer.pad_token)
    for i,a in enumerate(train_dataset):
        if counter==0:
            print(i,a)
            break
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            steps += 1
            if step %10 == 0:
                print_rank_0(f"Epoch: {epoch}, Total Step: {steps}, Loss: {loss}", args.global_rank)
            model.backward(loss)
            model.step()
        ...
        print_rank_0(
            ...
            args.global_rank)
        eval_loss, perplexity = evaluation(model, eval_dataloader)
        print_rank_0(f"ppl: {perplexity}", args.global_rank)
        print_rank_0(f"eval loss: {eval_loss}", args.global_rank)
